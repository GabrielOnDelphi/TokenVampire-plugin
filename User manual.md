# ClaudeTokenVampire — User Manual

ClaudeTokenVampire - monitors Claude Code's rolling 5-hour token usage window.
It lives in the system tray and periodically scans Claude's session files.
...
- **Right-click** tray icon: context menu (Settings, Exit)
- Closing the window (X button) hides it to tray — does not exit
- Tray tooltip shows current token usage summary
...
Five vertical bars — one per hour of the 5-hour window:

```
[4h ago] [3h ago] [2h ago] [1h ago] [now]
```

Each bar height = total tokens used in that hour.
...
ClaudeTokenVampire﻿﻿ reads JSONL files from `%USERPROFILE%\.claude\projects\`. It opens files in read-only shared mode so it never interferes with Claude Code. No data is sent anywhere.
